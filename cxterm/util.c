/* $Id: util.c,v 1.2 2003/05/06 07:40:19 htl10 Exp $ */
/***********************************************************************
...
void ClearRight (register TScreen *screen);
void ClearLeft (register TScreen *screen);
void ClearScreen (register TScreen *screen);
void CopyWait (register TScreen *screen);
void scrolling_copy_area (TScreen *screen, int firstline, int nlines, int amount);
void recolor_cursor (Cursor cursor, long unsigned int fg, long unsigned int bg);
...
extern void ScrnDeleteLine (register ScrnBuf sb, register int last, int where, register int n, register int size);
extern void ScrnInsertLine (register ScrnBuf sb, int last, register int where, register int n, register int size);
extern int non_blank_line (ScrnBuf sb, register int row, register int col, register int len);
extern void ScrnInsertChar (ScrnBuf sb, int row, register int col, register int n, int size);
extern void ScrnDeleteChar (ScrnBuf sb, register int row, register int col, register int n, register int size);
extern void ClearBufRows (register TScreen *screen, register int first, register int last);
int HandleExposure (register TScreen *screen, register XEvent *event);
int handle_translated_exposure (register TScreen *screen, register int rect_x, register int rect_y, register unsigned int rect_width, register unsigned int rect_height);
extern void RefreshCXtermInput (TScreen *screen);
extern void ChangeTekColors (register TScreen *screen, ScrnColors *pNew);
extern void set_cursor_gcs (TScreen *screen);
extern void TekExpose (Widget w, XEvent *event, Region region);
extern void ScrollBarReverseVideo (register Widget scrollWidget);
extern void TekReverseVideo (register TScreen *screen);
extern void ReverseOldColors (void);

void FlushScroll(register TScreen *screen)
{
	register int i;
...
int AddToRefresh(register TScreen *screen)
{
 ...
 * requires: amount > 0
 */
void Scroll(register TScreen *screen, register int amount)
{
 ...
 */
void InsertLine (register TScreen *screen, register int n)
{
	register int i;
	register int shift;
	register int bot;
 ...
 */
void DeleteLine(register TScreen *screen, register int n)
{
	register int i;
	register int shift;
	register int bot;
 ...
 */
void InsertChar (register TScreen *screen, register int n)
{
 ...
 */
void DeleteChar (register TScreen *screen, register int n)
{
	register int width;
 ...
 */
void ClearAbove (register TScreen *screen)
{
 ...
 */
void ClearBelow (register TScreen *screen)
{
	register int top;
 ...
 */
void ClearLine(register TScreen *screen)
{
	if(screen->cursor_state)
		HideCursor();
	screen->do_wrap = 0;
	if(screen->cur_row - screen->topline <= screen->max_row) {
		...
		XClearArea (screen->display, TextWindow(screen), 
		     screen->border + screen->scrollbar,
		      CursorY (screen, screen->cur_row),
		     Width(screen), FontHeight(screen), FALSE);
	    }
	}
...
void ChangeColors(XtermWidget term, ScrnColors *pNew)
{
	register TScreen *screen = &term->screen;
...
void ReverseVideo (XtermWidget termw)
{
