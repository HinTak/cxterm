/* $Id: button.c,v 1.2.2.2 2004/08/04 18:48:09 htl10 Exp $ */
/***********************************************************************
...
extern void DoSecureKeyboard(Time);
...
void TrackText (register int frow, register int fcol, register int trow, register int tcol);
void DisownSelection (register XtermWidget termw);
...
void ScrollSelection(register TScreen *screen, register int amount)
{
...
static void
PointToRowCol(register int y, register int x, int *r, int *c)
...
int SetCharacterClassRange (register int low, register int high, register int value)
                           		/* in range of [0..255] */
                       			/* arbitrary */
{
...
#define RowCol_HZ(r,c)			\
		(((c) <= screen->max_col) &&	\
		 ((screen->buf[4 * ((r)+screen->topline) + 1][c]) & CHARHANZI))
#define RowCol_2ndByteHZ(r,c)		\
		(((c) <= screen->max_col) &&	\
		 Is2ndByteHZ(screen->buf[4 * ((r)+screen->topline) + 1][c]))
...
static void
ReHiliteText(register int frow, register int fcol, register int trow, register int tcol)
...
static Boolean ConvertSelection(Widget w, Atom *selection, Atom *target, Atom *type, XtPointer *value, long unsigned int *length, int *format)
{
...
/* ARGSUSED */
static int
Length(register TScreen *screen, register int row, register int scol, register int ecol)
{
...
static char *
SaveText(TScreen *screen, int row, int scol, int ecol, register char *lp, int *eol)
...
/*ARGSUSED*/
void HandleGINInput (Widget w, XEvent *event, String *param_list, Cardinal *nparamsp)
{
