/* $Id: input.c,v 1.2 2003/05/06 07:40:19 htl10 Exp $ */
/***********************************************************************
...
# if (X11RELEASE <= 5) && !defined(XK_KP_Home)
...
extern void unparseseq (register ANSI *ap, int fd);
extern void unparseputc (char c, int fd);
...
void Input (register TKeyboard *keyboard, register TScreen *screen, register XKeyEvent *event, int eightbit)
{
...
#define STRBUFSIZE 100
    ...
    /* If there's any key pressed during waiting for release the toggle-key,
       we should reset the toggle-key status.
     */
    g_WaitingToggle = 0;
...
void StringInput (register TScreen *screen, register char *string, int nbytes)
{
	int	pty	= screen->respond;
